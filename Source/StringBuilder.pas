...
type
  ...
  private
    ...
    begin
      ...
      {$IFDEF WINDOWS}ExternalCalls.{$ELSEIF POSIX OR BAREMETAL}rtl.{$ELSE}{$ERROR}{$ENDIF}memcpy(Dest, Source, CharCount*sizeOfChar);
    end;

    method IntMove(SourceIndex, DestIndex: Int32;  CharCount: Integer);
    begin
      if CharCount = 0 then exit;
      {$IFDEF WINDOWS}ExternalCalls.{$ELSEIF POSIX OR BAREMETAL}rtl.{$ELSE}{$ERROR}{$ENDIF}memmove(@fBuf[DestIndex], @fBuf[SourceIndex], CharCount*sizeOfChar);
    end;
