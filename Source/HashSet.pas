      ...
      var new_fbucketTable := new array of Integer(aNewCapacity);
{
      ...
      if fMaxUsedIndex > DEFAULT_MAX_INDEX then begin
        {$IFDEF WINDOWS}ExternalCalls.{$ELSEIF POSIX OR BAREMETAL}rtl.{$ELSE}{$ERROR}{$ENDIF}memmove(@new_fEntriesTable[0], @fEntriesTable[0], fMaxUsedIndex * sizeOf(HashEntry<T>));
      end;
