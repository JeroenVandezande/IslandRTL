      ...
      var new_fbucketTable := new array of Integer(aNewCapacity);
{
      ...
      if fMaxUsedIndex > DEFAULT_MAX_INDEX then begin
        {$IFDEF WINDOWS}ExternalCalls.{$ELSEIF POSIX}rtl.{$ELSEIF NOGC}rtl.{$ELSE}{$ERROR}{$ENDIF}memmove(@new_fEntriesTable[0], @fEntriesTable[0], fMaxUsedIndex * sizeOf(Entry<T,U>));
      end;
